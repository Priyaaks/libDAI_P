...
NAMES:=graph dag bipgraph varset daialg alldai clustergraph factor factorgraph properties regiongraph util weightedgraph exceptions exactinf evidence emalg emrun io
#NAMES:= layer emrun
ifdef WITH_BP
...
HEADERS=$(foreach name, emrun, $(INC)/$(name).h)
...
#EXAMPLES=$(foreach name, example example_mtem example_varset example_Newasia_mtem computeDistance doInference 2NodeBN_createfg normal_distribution Movie3Node1_createfg 3NodeBN1_createfg generaterandomFg generateMovieFg GaemGlobal GaemGlobalRandom example_sprinkler example_Newasia_em example_gaem_deterministic example_gaem_deterministic_distance example_gaem_deterministic_distance_pc uai2010-aie-solver testemrun,examples/$(name)$(EE))
EXAMPLES=$(foreach name, example bimap midifybimap example_varset example_mtem example_varset example_sprinkler example_Newasia_em doInference computeDistance example_gaem_deterministic,examples/$(name)$(EE))
#EXAMPLES=$(foreach name, example_gaem_deterministic ,examples/$(name)$(EE))

ifdef WITH_GIBBS
EXAMPLES:=$(EXAMPLES) examples/example_sprinkler_gibbs$(EE)
#EXAMPLES:=$(EXAMPLES) examples/example_asia_gibbs$(EE)
#XAMPLES:=$(EXAMPLES) examples/example_asia_gibbs_h4$(EE)
#EXAMPLES:=$(EXAMPLES) examples/example_asia_gibbs_h8$(EE)
#EXAMPLES:=$(EXAMPLES) examples/example_asia_jtree$(EE)
endif
ifdef WITH_CIMG
