...
const size_t EMAlg::MAX_ITERS_DEFAULT = 10000;
const Real EMAlg::LOG_Z_TOL_DEFAULT = 0.00001;
...
Real EMAlg::iterate( MaximizationStep &mstep ) {
    ...
    _estep.run();
    logZ = _estep.logZ();
   // cout<<"_max_iters"<<_max_iters<<endl;
	...
	//ii=0;
        ...
        for( Evidence::Observation::const_iterator i = e->begin(); i != e->end(); ++i )
		 ...
		 ii=ii+1;		
		 //cout<<ii<<" "<<clamped->fg().findVar(i->first)<<" "<<i->second<<endl;
	   }
	    ...
	    //cout<<"clamped->logZ()"<<clamped->logZ()<<" likelihood"<<likelihood<<endl;
...
Real EMAlg::iterate() {
    Real likelihood;
    //cout<<_msteps.size()<<endl;
    for( size_t i = 0; i < _msteps.size(); ++i )
	{
        likelihood = iterate( _msteps[i] );
	//cout<<"final likelihood"<<likelihood<<endl;
	}
