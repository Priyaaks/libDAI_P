...
void Evidence::addEvidenceTabFile( std::istream &is, std::map<std::string, Var> &varMap ) {
    ...
    //cout<<"Headersize = "<<header_fields.size()<<endl;
    ...
    while( getline(is, line) ) {
       ...
       // cout<<"checking line "<<line_number<<endl;
        ...
        for( size_t i = 0; i < vars.size(); ++i ) {
            if( fields[i].size() > 0 ) { // skip if missing observation
                ...
                if( state >= vars[i].states() )
                    DAI_THROWE(INVALID_EVIDENCE_FILE,"State "+boost::lexical_cast<std::string>(i)+" " + fields[i] + " too large in line " + boost::lexical_cast<std::string>(line_number));
                sample[vars[i]] = state;
            }
        }
